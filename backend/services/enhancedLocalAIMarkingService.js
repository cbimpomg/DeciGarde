...
// Enhanced Local AI marking service with actual models
const markScript = async (scriptId) => {
  try {
    console.log(`Starting enhanced local AI marking for script: ${scriptId}`);
    ...
    // Mark each question with enhanced local AI algorithms
    ...
    console.log(`Enhanced local AI marking completed for script: ${scriptId}`);
    ...
    console.error('Enhanced local AI marking error:', error);
...
// Enhanced question marking with local AI models
const markQuestionEnhanced = async (question, fullText) => {
  try {
    ...
    // Multiple enhanced local AI approaches
    const results = {
      ...
      tfidf: await analyzeWithTFIDF(question, answerText, maxScore)
    };
...
const extractAnswerForQuestion = (questionText, fullText) => {
  ...
  const questionTokens = tokenizer.tokenize(questionText.toLowerCase());
  const sentences = fullText.split(/[.!?]+/);
  
  // Find sentences that contain question keywords
  const relevantSentences = sentences.filter(sentence => 
    questionTokens.some(token => sentence.includes(token))
  );
  
  return relevantSentences.join(' ').trim();
};

// Enhanced keyword analysis with TF-IDF
...
// Enhanced semantic similarity using TF-IDF
const analyzeWithSemanticSimilarity = async (question, answerText, maxScore) => {
  try {
    // Use TF-IDF for semantic similarity
    const tfidf = new natural.TfIdf();
    tfidf.addDocument(question.questionText);
    tfidf.addDocument(answerText);
    
    // Calculate cosine similarity
    const questionTerms = tfidf.listTerms(0);
    const answerTerms = tfidf.listTerms(1);
...
// TF-IDF analysis for better content understanding
const analyzeWithTFIDF = async (question, answerText, maxScore) => {
  try {
    const tfidf = new natural.TfIdf();
    tfidf.addDocument(answerText);
    ...
    const importantTerms = terms.filter(term => term.score > 0.1);
    
    let score = 0;
    const questionTokens = tokenizer.tokenize(question.questionText.toLowerCase());
    
    // Score based on important terms that match question
    for (const term of importantTerms) {
      if (questionTokens.includes(term.term)) {
        score += maxScore * 0.2;
      }
    }
    ...
    return {
      score: Math.min(score, maxScore),
      confidence: importantTerms.length / terms.length
    };
  } catch (error) {
    console.error('TF-IDF analysis error:', error);
    return { score: 0, confidence: 0 };
  }
};

// Enhanced content analysis using local AI
const analyzeWithContentAnalysis = async (question, answerText, maxScore) => {
  try {
    ...
    return { 
      score: maxScore * 0.7, 
      feedback: 'Enhanced local AI analysis completed', 
      confidence: 0.7 
    };
  }
};
...
const calculateCosineSimilarity = (vec1, vec2) => {
  const dotProduct = vec1.reduce((sum, val, i) => sum + val.score * (vec2[i]?.score || 0), 0);
  const magnitude1 = Math.sqrt(vec1.reduce((sum, val) => sum + val.score * val.score, 0));
  const magnitude2 = Math.sqrt(vec2.reduce((sum, val) => sum + val.score * val.score, 0));
...
// Enhanced essay answer analysis
...
// Enhanced definition answer analysis
...
// Enhanced general answer analysis
...
const generateComprehensiveFeedback = (results, answerText) => {
  ...
  if (results.tfidf.confidence > 0.5) {
    feedbacks.push('Answer contains relevant important terms');
  }
