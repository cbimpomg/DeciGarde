...
function useSwitchColors(highlighted) {
  ...
  const backgroundColor = React.useMemo(() => {
    if (theme.dark) {
      ...
      if (theme.isV3) {
        return (0, _color.default)(theme.colors.surface).lighten(1.4).hex();
      }
...
function useInputColors(highlighted) {
  ...
  const backgroundColor = React.useMemo(() => {
    ...
    if (highlighted) {
      if (theme.isV3) {
        return (0, _color.default)(theme.colors.primaryContainer).hex();
      }
    ...
    if (theme.isV3) {
      return (0, _color.default)(theme.colors.outline).lighten(0.9).hex();
    }
